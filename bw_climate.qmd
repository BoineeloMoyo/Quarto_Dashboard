...
# Load your data
ghg <- read_csv("~/GIT/daten_import/1_datenpruefung/rcode/datenberdarfstab_cq/bw_ghg.csv") # Adjust path if needed
read_csv("./data/bw_ghg.csv")
# Convert emissions to million metric tonnes
ghg <- ghg %>%
  mutate(across(-Year, ~ .x / 1e6)) %>%
  rename(`CO2` = CO2, `N₂O` = N20, `CH₄` = CH4, `F-Gases` = F_Gas) %>%
  select(Year, CO2, `N₂O`, `CH₄`, `F-Gases`)  # reorder to show F-Gases last (drawn on top)
...
ghg |>
  e_charts(Year) |>
  e_river(CO2, name = "CO2") |>
  e_river(`N₂O`, name = "N₂O") |>
  e_river(`CH₄`, name = "CH₄") |>
  e_river(`F-Gases`, name = "F-Gases") |>
  e_color(teal_pal) |>
  ...
  e_axis_labels(x = "", y = "") |>  # clean axis labels since we already added titles
