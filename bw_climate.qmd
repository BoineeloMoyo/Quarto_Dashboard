...
hazards <- data.frame(
  Hazard = c("Flood", "Drought", "Epidemic", "Storm"),
  Percentage = c(55.56, 22.22, 16.67, 5.56),
  Color = c("#1e3435", "#2d5e5b", "#418186", "#77aeb2")  # Corrected colors
)
...
hazards |>
  ...
  e_pie(Percentage, 
        radius = c("50%", "70%"),  
        label = list(show = TRUE),  # Keep default labels
        itemStyle = list(borderRadius = 10)) |>
  e_tooltip(
    trigger = "item",
    formatter = "{b}: {c}%"  # Adds % sign to tooltip
  ) |>
  e_color(hazards$Color) |>  # Apply corrected colors
  e_title("Botswana Natural Hazard Occurrence") |> 
  e_legend(show = FALSE) |>  # Ensure legend is removed
  e_text_style(fontSize = 14, fontWeight = "bold")
