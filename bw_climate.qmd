...
### Row {height=50%}
```{r co2_emissions}
#| title: GHG Emissions in in metric tonnes of CO2
...
donut_plotly <- plot_ly(
  ...
  layout(
    title = list(
      text = htmltools::HTML("Greenhouse Gas Emissions CO<sub>2</sub></span> by Sector in Botswana<br><span style='font-size:12px;'> "), 
      x = 0.5
    ),
    showlegend = FALSE,
    annotations = list(
      list(
        ...
        showarrow = FALSE,
        font = list(size = 22, color = "#1e3435")  # Adjust font size & color
      )
    )
  )
